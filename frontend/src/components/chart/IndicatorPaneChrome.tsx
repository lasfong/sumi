...
export const IndicatorPaneChrome: React.FC<Props> = ({ document, runtime, onSettings, onToggle, onRemove }) => {
  ...
  // Group visible non-price instances by physical paneId, preserving pane order
  const paneGroupsMap = new Map<string, typeof visible>();
  for (const instance of visible) {
    const list = paneGroupsMap.get(instance.paneId) ?? [];
    list.push(instance);
    paneGroupsMap.set(instance.paneId, list);
  }
  const paneGroups = Array.from(paneGroupsMap.entries());
  ...
  return <div data-testid="indicator-pane-chrome-layer" aria-label="Visible indicator pane controls" style={{ position: 'absolute', inset: '0 68px 26px 0', zIndex: 6, pointerEvents: 'none', display: 'flex', flexDirection: 'column' }}>
    ...
    {paneGroups.map(([paneId, instances]) => {
      const primaryInstance = instances[0];
      return <section
        key={paneId}
        data-testid={`indicator-pane-${primaryInstance.definitionId}`}
        data-pane-instance-id={primaryInstance.id}
        data-pane-id={paneId}
        data-status={runtime[primaryInstance.id]?.status ?? 'idle'}
        aria-label={`${instances.map(i => i.label).join(' & ')} pane`}
        style={{ flex: '1 1 0', minHeight: 60, position: 'relative', borderTop: '1px solid rgba(255,255,255,.12)' }}
      >
        <div style={{ pointerEvents: 'auto', position: 'absolute', top: 3, left: 5, maxWidth: 'calc(100% - 10px)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '4px 8px' }}>
          {instances.map(instance => {
            ...
            return <div
              key={instance.id}
              data-testid={`indicator-pane-chrome-${instance.id}`}
              data-status={state.status}
              style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '2px 7px', padding: '2px 5px', borderRadius: 4, background: 'rgba(13,17,23,.78)', color: '#F0F6FC', fontSize: 10, lineHeight: 1.35 }}
            >
