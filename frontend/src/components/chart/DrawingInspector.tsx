...
export const DrawingInspector: React.FC<Props> = ({ selected, persistenceStatus, onApply, onDelete }) => {
  ...
  return <section data-testid="drawing-selection-toolbar" aria-label={`Selected ${selected.tool} ${selected.id}`} onKeyDownCapture={stopChartKeys}
    ...
    <fieldset style={{ display: 'grid', gap: 8, minWidth: 0 }}><legend>Anchors</legend>
      {draft.anchors.map((anchor, index) => <div key={index} style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gap: 6 }}>
        <label style={{ display: 'grid', gap: 3 }}>Anchor {index + 1} date
          <input aria-label={`Anchor ${index + 1} time`} data-testid={`drawing-anchor-${index}-time`} type="date" value={anchor.time} onChange={event => updateAnchor(index, 'time', event.target.value)} style={controlStyle} />
        </label>
        <label style={{ display: 'grid', gap: 3 }}>Anchor {index + 1} price
          <input aria-label={index === 0 ? 'Selected line price' : `Anchor ${index + 1} price`} data-testid={index === 0 ? 'drawing-price-input' : `drawing-anchor-${index}-price`}
    ...
    {(draft.tool === 'rectangle' || draft.tool === 'fibonacci-retracement') && <fieldset style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 8, minWidth: 0 }}><legend>Fill</legend>
